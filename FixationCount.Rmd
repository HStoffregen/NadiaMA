---
title: "Fixation Count - corrected&related - MC"
...
model <- lm(FixationCount ~ bedingung*Type, data=dfLM)
...
```{r}
model <- lm(FixationCount ~ bedingung*Type*anreiz, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(FixationCount ~ bedingung*Type*commitment, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(FixationCount ~ bedingung*Type*erwartung, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(FixationCount ~ bedingung*Type*attention, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(FixationCount ~ bedingung*Type*freeRecall, data=dfLM)
summary(model)
```
#### Recognition - Hits

```{r}
model <- lm(FixationCount ~ bedingung*Type*Hits, data=dfLM)
summary(model)
```
