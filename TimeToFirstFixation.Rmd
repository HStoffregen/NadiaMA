---
title: "Time To First Fixation (TTFF) - corrected&related - MC"
...
-	**correct for within subjects variables picture, Type, and Order:** adjustment factor (= group mean - individual mean) is subtracted from raw scores
-	**scores are given as a relation to total scores per group:** individual score(s)/group mean
...
df$grandMean <- NA
df[c(df$bedingung=="MC"),]$grandMean <- mean(df[c(df$bedingung=="MC"),]$TTFF, na.rm=T)
df[c(df$bedingung=="IN"),]$grandMean <- mean(df[c(df$bedingung=="IN"),]$TTFF, na.rm=T)
df$pMean <- NA

for (i in c(unique(df$vpn))){
  df[c(df$vpn==i),]$pMean <- mean(df[c(df$vpn==i),]$TTFF, na.rm=T)
}
df$adj <- df$grandMean - df$pMean

if (adjust == TRUE){
  df$TTFF <- df$TTFF + df$adj
}

#relate scores to mean in group
if (relateScores == TRUE){
 df$TTFF <- df$TTFF/df$grandMean
}
...
dfDiff <- dcast(df,  vpn+Picture+Order+bedingung+erwartung+anreiz+commitment+attention+freeRecall+Hits ~ Type, value.var = "TTFF", mean)
...
dfDiff <- melt(dfDiff, measure.vars = c(14:16), variable.name = "DiffScore", value.name = "TTFF")
#rename Order
...
Graph <- df
Graph <- Graph[!is.na(Graph$TTFF),]

Graph <- Graph %>%
    group_by(Picture, Order, bedingung, Type) %>%
    summarise(mean.TTFF = mean(TTFF), sd.TTFF = sd(TTFF), se.TTFF = sd(TTFF)/sqrt(n()), n=n()) 

gp <- ggplot(data = Graph,
             aes(x=Type, y=mean.TTFF, colour=Order))
#set points
gp <- gp + geom_point()
...
Graph <- Graph %>%
  ...
  mutate(outlier = ifelse(is_outlier(mean.TTFF),Picture, as.numeric(NA))) 

```
```{r, fig.width = 12}
gp <- ggplot(data = Graph,
             aes(x=Order, y=mean.TTFF, colour=bedingung))
#set points
gp <- gp + geom_boxplot()
...
gp <- ggplot(data = Graph,
             aes(x=Picture, y=mean.TTFF, colour=bedingung))
#set points
gp <- gp + geom_point()
...
#df1 <- df %>%
  ...
  #summarise(TTFF = mean(TTFF, na.rm=T)) 
```
```{r, fig.width = 12, fig.height = 6}
gp <- ggplot(df, aes(anreiz, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
...
gp <- ggplot(df, aes(erwartung, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
gp <- gp+facet_wrap(~Type)
...
gp <- ggplot(df, aes(commitment, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
gp <- gp+facet_wrap(~Type)
...
gp <- ggplot(df, aes(attention, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
gp <- gp+facet_wrap(~Type)
...
gp <- ggplot(df, aes(freeRecall, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
gp <- gp+facet_wrap(~Type)
...
gp <- ggplot(df, aes(Hits, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
gp <- gp+facet_wrap(~Type)
...
```{r}
dfLM$TTFF[is.nan(dfLM$TTFF)] <- NA
dfLM <- dfLM[!is.na(dfLM$TTFF),]

model <- lm(TTFF ~ bedingung*Type, data=dfLM)
...
```{r}
model <- lm(TTFF ~ bedingung*Type*anreiz, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(TTFF ~ bedingung*Type*commitment, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(TTFF ~ bedingung*Type*erwartung, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(TTFF ~ bedingung*Type*attention, data=dfLM)
summary(model)
```
...
```{r}
model <- lm(TTFF ~ bedingung*Type*freeRecall, data=dfLM)
summary(model)
```
#### Recognition - Hits

```{r}
model <- lm(TTFF ~ bedingung*Type*Hits, data=dfLM)
summary(model)
```
...
```{r, fig.width = 12, fig.height = 6}
gp <- ggplot(dfDiff, aes(anreiz, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
#set colour
...
gp <- ggplot(dfDiff, aes(erwartung, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
#set colour
...
gp <- ggplot(dfDiff, aes(commitment, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
#set colour
...
gp <- ggplot(dfDiff, aes(attention, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
#set colour
...
gp <- ggplot(dfDiff, aes(freeRecall, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
#set colour
...
gp <- ggplot(dfDiff, aes(Hits, TTFF, colour=bedingung))
#set scatter plot
gp <- gp + geom_point()
gp <- gp + geom_smooth(method = "lm", aes(fill=bedingung), alpha = 0.1)
#set colour
...
```{r}
dfDiffLM <- dfDiffLM[!is.na(dfDiff$TTFF),]
dfDiffLM <- dcast(dfDiffLM,  vpn+Picture+bedingung+erwartung+anreiz+commitment+Order+attention+freeRecall+Hits ~ DiffScore, value.var = "TTFF", mean)
